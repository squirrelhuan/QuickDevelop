...
public class QdActionBarBehavior extends CoordinatorLayout.Behavior<QdActionBarContainer> {
     ...
     * @return
     */
    private QdActionBarContainer getPreviousChild(CoordinatorLayout parent, QdActionBarContainer child) {
    ...
    @Override
    public boolean onNestedPreFling(@NonNull CoordinatorLayout parent, @NonNull QdActionBarContainer child, @NonNull View target, float velocityX, float velocityY) {
    ...
    private int scrollParent(CoordinatorLayout parent, QdActionBarContainer child, View target, int dy) {
        ...
        if (dy > 0) {//上推
            ...
            QDLogger.d("下拉");
            QdActionBarContainer downer = getDownerChild(parent);//下一个
     ...
     * @return
     */
    private QdActionBarContainer getDownerChild(CoordinatorLayout parent) {
     ...
     * @return
     */
    private int scrollChild(CoordinatorLayout parent, QdActionBarContainer child, View target, int dy) {
        //1.优先parent滑动
        //2.寻找到可以滚动的下一个child
        return 0;
    }
    ...
    private int scroll(CoordinatorLayout parent, QdActionBarContainer child, int dy) {
        //1.k控制自己的移动
        int initialOffset = child.getTop();
        QdActionBarContainer firstchild = getFirstChild(parent);
        QdActionBarContainer lastchild = getLastChild(parent);
     ...
     * @return
     */
    private QdActionBarContainer getNextChild(CoordinatorLayout parent, QdActionBarContainer child) {
        int cartindex = parent.indexOfChild(child);
     ...
     * @return
     */
    private QdActionBarContainer getFirstChild(CoordinatorLayout parent) {
        QdActionBarContainer firstchild = null;
        for (int i = 0; i < parent.getChildCount(); i++) {
            View view = parent.getChildAt(i);
            if (view instanceof QdActionBarContainer) {
                firstchild = (QdActionBarContainer) view;
                return firstchild;
            }
        }
        return firstchild;
     ...
     * @return
     */
    private QdActionBarContainer getCurrentChild(CoordinatorLayout parent) {
        QdActionBarContainer current = null;
        for (int i = 0; i < parent.getChildCount(); i++) {
            View view = parent.getChildAt(i);
            if (view.getTop() >= 0) {
                return (QdActionBarContainer) view;
            }
        }
        return current;
    }
     ...
     * @return
     */
    private QdActionBarContainer getLastChild(CoordinatorLayout parent) {
        QdActionBarContainer lastchild = null;
        for (int i = parent.getChildCount() - 1; i >= 0; i--) {
            View view = parent.getChildAt(i);
            if (view instanceof QdActionBarContainer) {
                lastchild = (QdActionBarContainer) view;
                return lastchild;
            }
        }
        return lastchild;
     ...
     * @param offset
     */
    private void moveOther(CoordinatorLayout parent, QdActionBarContainer child, int offset) {
        QdActionBarContainer current = child;
        QdActionBarContainer above = getPreviousChild(parent, current);
        while (above != null) {
        ...
        current = child;
        QdActionBarContainer below = getNextChild(parent, current);
        while (below != null) {
    ...
    @Override
    public boolean onTouchEvent(@NonNull CoordinatorLayout parent, @NonNull QdActionBarContainer child, @NonNull MotionEvent event) {
    ...
    private void constrainScrollBy(int dy) {
       /* Rect viewport = new Rect();
        ...
        QdActionBarContainer child = getCurrentChild(parent);
