...
/*
    ...
    public void chekPermission(Context context, String[] permissions, PermissionManager.OnCheckPermissionListener listener) {
        // 版本判断。当手机系统大于 23 时，才有必要去判断权限是否获取
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            //用map去重
            ...
            if (list.size() > 0) {
                ...
                if (specialMap.containsKey(str)) {//处理特殊权限
                    ...
                    if (getPermissionStatusByName(context, str)) {//检查特殊权限
                        ...
                        showPermissionSettingDialog(context, str, new PermissionManager.OnCheckPermissionListener() {
                            @Override
                    ...
                    chekSignPermission(context, targest, new PermissionManager.OnCheckPermissionListener() {
                        @Override
                        public void onPassed() {
    ...
    public void chekSignPermission(Context context, String[] permissions, PermissionManager.OnCheckPermissionListener listener) {
        // 版本判断。当手机系统大于 23 时，才有必要去判断权限是否获取
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            // 检查该权限是否已经获取
    ...
    private static Map<Integer, PermissionManager.OnCheckPermissionListener> requestMap = new HashMap<>();

    // 开始提交请求权限
    private void startRequestPermission(final Context context, String[] permissions, PermissionManager.OnCheckPermissionListener listener) {
        ...
        if (REQUEST_PERMISS_COMMON_CODE == requestCode) {
            ...
            if (listener != null) {
                ...
                if (hasPermissionDismiss) {
                    listener.onNoPassed();//跳转到系统设置权限页面，或者直接关闭页面，不让他继续访问
                } else {
                    //全部权限通过，可以进行下一步操作。。。
                    listener.onPassed();
                }
