...
package cn.demomaster.huan.quickdeveloplibrary.view.colorpicker;
...
public class ColorPicker extends View {
    ...
    protected void onDraw(Canvas canvas) {
      ...
      /*  float[] hsv = new float[] { colorHSV[0], colorHSV[1], 1f };
       ...
       /* valuePointerPaint.setColor(Color.HSVToColor(new float[] { 0f, 0f, 1f - colorHSV[2] }));
        ...
        float valueAngleX = (float) Math.cos(valueAngle);
        float valueAngleY = (float) Math.sin(valueAngle);
        if (valueSliderRect != null) {
            float[] hsv = new float[]{colorHSV[0], colorHSV[1], 1f};
            LinearGradient linearGradient = new LinearGradient(valueSliderRect.left, valueSliderRect.top, valueSliderRect.right, valueSliderRect.bottom, new int[]{Color.WHITE, Color.HSVToColor(hsv), Color.BLACK}, null, Shader.TileMode.REPEAT);
            valueSliderPaint.setShader(linearGradient);
            canvas.drawRoundRect(valueSliderRect, 0, 0, valueSliderPaint);
            Paint paint = new Paint();
            paint.setColor(Color.BLACK);
            paint.setStyle(Style.STROKE);
            canvas.drawRoundRect(valueSliderRect, 0, 0, paint);
        ...
        if (alphaSliderRect != null) {
            int cellHeight = (int) (alphaSliderRect.height() / 3f);
            int count = (int) (alphaSliderRect.width() / cellHeight);
            for (int j = 0; j < 3; j++) {
                for (int i = 0; i < count; i++) {
                    float left = alphaSliderRect.left + cellHeight * i;
                    float top = alphaSliderRect.top + cellHeight * j;
                    Paint paint = new Paint();
                    paint.setColor(Color.WHITE);
                    if (j % 2 == 0) {
                        if (i % 2 == 0) {
                    ...
                    canvas.drawRect(left, top, left + cellHeight, top + cellHeight, paint);
                }
            }

            float[] hsv = new float[]{colorHSV[0], colorHSV[1], 1f};
            LinearGradient linearGradient = new LinearGradient(alphaSliderRect.left, alphaSliderRect.top, alphaSliderRect.right, alphaSliderRect.bottom, new int[]{Color.TRANSPARENT, Color.HSVToColor(hsv)}, null, Shader.TileMode.REPEAT);
            valueSliderPaint.setShader(linearGradient);
            canvas.drawRoundRect(alphaSliderRect, 0, 0, valueSliderPaint);

            Paint paint = new Paint();
            paint.setColor(Color.BLACK);
            paint.setStyle(Style.STROKE);
            canvas.drawRoundRect(alphaSliderRect, 0, 0, paint);
    ...
    protected void onSizeChanged(int width, int height, int oldw, int oldh) {
        ...
        int top = 30;
        //颜色滑块
        valueSliderRect = new RectF(centerX - colorWheelRadius, centerY + colorWheelRadius + top, centerX + colorWheelRadius, centerY + colorWheelRadius + top + sliderBarHeight);
        //透明度滑块
        alphaSliderRect = new RectF(valueSliderRect.left, valueSliderRect.bottom + top, valueSliderRect.right, valueSliderRect.bottom + top + sliderBarHeight);
    }

    int sliderBarHeight = 60;
    //透明度选择
    RectF valueSliderRect;
    RectF alphaSliderRect;
